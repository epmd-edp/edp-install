...
Pay attention that secret names should be the same as the 'credentialName' property in JiraServer/PerfServer custom resources and the 'nameSshKeySecret' property for GIT.

> **INFO**: If your system requires to use Luminate, pay attention that the secret name must be the same as the **perf-operator.perf.luminate.credentialName**.
