...
Assuming that cluster is running under the account, for instance, **712239771395-compute@developer.gserviceaccount.com**
Further, we will refer to that account as **cluster_sa**
...
*Note: You probably need to request a support group to do that

2. Grant that role to **cluster_sa** . 
...
3. Create a key file and save it with the name **kaniko-secret.json**. It can be done via going to "IAM -> Service Accounts"
                                                                        
![go_to_IAM](../documentation-resources/go_to_IAM.png "go_to_IAM")

finding there **cluster_sa**. The at the right end of the corresponding record there will be three vertical dots:

![go_to_Create_key.png](../documentation-resources/go_to_Create_key.png "go_to_Create_key.png")

Click "Create key" and then click "CREATE"
![go_Create.png](../documentation-resources/go_Create.png "go_Create.png")
 
   *Note: Keep it privately. It will help you to authorize Kaniko while accessing a Google docker registry*

4. Go to the cluster console and create a Secret with the name **kaniko-secret** in namespace **demo-edp-cicd** from the previous file. 
   ```
   kubectl -n demo-edp-cicd create secret generic kaniko-secret --from-file kaniko-secret.json
   ```
5. Then create a ConfigMap object with the name **kaniko-template** in namespace **demo-edp-cicd**  via the below command 
   
   ```
   cat <<EOF | kubectl -n demo-edp-cicd apply -f -
   apiVersion: v1
   data:
     kaniko.json: |-
       {
   ...
   EOF
   ```
6. Before running pipelines, you should setup default Google container registry, being computed by the formula **gcr.io**/**GCP_Project_ID**, for instance, **gcr.io/or2-msq-epmd-edp-t1iylu** 
   We will define that registry via creating a EDPComponent:
   ...
   EOF
   ```
7. One can find images in the following menu
