# Google container registry (GCR) Integration

**Prerequisetes**:

Assuming that cluster is running under the GCP service account, for instance, **712239771395-compute@developer.gserviceaccount.com**
(**cluster_sa**)

Discover the steps below to apply the GCR integration correctly:

1. Create a custom role, for instance, **Storage_C1714468**, for the **cluster_sa** with the following permissions:

    * storage.buckets.create
    * storage.buckets.delete
    * storage.buckets.get
    * storage.buckets.list
    * storage.buckets.update
    * storage.objects.create
    * storage.objects.delete
    * storage.objects.get
    * storage.objects.list
    * storage.objects.update

*Note: You probably need to request these permission via support team

2. Grant that role to **cluster_sa** . 

Check the role is assigned:

![go_Grant_custom_role](../documentation-resources/go_Grant_custom_role.png "go_Grant_custom_role")
 
*Note: You probably need to request this assignment via support team

3. Create a key file and save it with the name **kaniko-secret.json**. It can be done via going to "IAM -> Service Accounts"
                                                                        
![go_to_IAM](../documentation-resources/go_to_IAM.png "go_to_IAM")

finding there **cluster_sa**. The at the right end of the corresponding record there will be three vertical dots:

![go_to_Create_key.png](../documentation-resources/go_to_Create_key.png "go_to_Create_key.png")

Click "Create key" and then click "CREATE"

![go_Create.png](../documentation-resources/go_Create.png "go_Create.png")
 
   *Note: Keep it privately. It will help you to authorize Kaniko while accessing a Google Container Registry*

4. Go to the cluster console and create a Secret with the name **kaniko-secret** in namespace **demo-edp-cicd** from the previous file.
   ```
   kubectl -n demo-edp-cicd create secret generic kaniko-secret --from-file kaniko-secret.json
   ```
5. Then create a ConfigMap object with the name **kaniko-template** in namespace **demo-edp-cicd**  by applying the following template:
   
   ```
   apiVersion: v1
   data:
     kaniko.json: |-
       {
         "apiVersion": "v1",
         "kind": "Pod",
         "metadata": {
           "name": "kaniko"
         },
         "spec": {
           "initContainers": [
             {
               "name": "init-kaniko",
               "image": "busybox",
               "command": [
                 "/bin/sh",
                 "-c",
                 "while ! [[ -f /tmp/workspace/Dockerfile ]]; do echo \"Waiting for Dockerfile in workspace...\"; sleep 10;done"
               ],
               "volumeMounts": [
                 {
                   "name": "shared-volume",
                   "mountPath": "/tmp/workspace"
                 }
               ]
             },
           ],
           "containers": [
             {
               "name": "kaniko",
               "image": "gcr.io/kaniko-project/executor:v0.13.0",
               "env": [
                 {
                   "name": "GOOGLE_APPLICATION_CREDENTIALS",
                   "value": "/secret/kaniko-secret.json"
                 }
               ],
               "args": [
                 "--destination=REPLACE_DESTINATION_IMAGE"
               ],
               "volumeMounts": [
                 {
                   "name": "shared-volume",
                   "mountPath": "/workspace"
                 },
                 {
                   "name": "kaniko-secret",
                   "mountPath": "/secret"
                 }
               ]
             }
           ],
           "restartPolicy": "Never",
           "volumes": [
             {
               "name": "shared-volume",
               "emptyDir": {}
             },
             {
               "name": "kaniko-secret",
               "secret": {
                 "secretName": "kaniko-secret"
               }
             }
           ]
         }
       }
   kind: ConfigMap
   metadata:  
     name: kaniko-template 
   ```
6. Before running pipelines, you should create EDP Component that point to Google Contatiner Registry URL, that will be **gcr.io/<GCP_Project_ID>**. For instance, **gcr.io/or2-msq-epmd-edp-t1iylu**

Use the following template to create EDP Component:

   ```
   apiVersion: v1.edp.epam.com/v1alpha1
   kind: EDPComponent
   metadata:
     name: docker-registry
   spec:
     icon: 
     type: docker-registry
     url: gcr.io/or2-msq-epmd-edp-t1iylu
   ```
7. One can find built images in Container Registry section of GCP Web console.

![gcr_images](../documentation-resources/gcr_images.png "gcr_images")