...
and a password or use the existing secret. According to our templates, it is possible to use the secret export command: 
```bash
...
* Install EDP in the <edp-project> project with the helm command. Depending on the cloud provider, the parameter values may differ, make sure that the set values​are correct for your provider. Find below the installation basic command example:
```bash
    helm install <helm_release_name> epamedp/edp-install --version "2.4.0" --wait --timeout=900s --namespace <edp-project> \
    ...
    --set global.webConsole=<cluster_webConsole_url> \
    --set edp.adminGroups={<edp-project>-admin-group} \
    --set edp.developerGroups={<edp-project>-admin-group} \
